...
Focussing on the transmitting path, the need for a \gls{ab:dac} becomes visible, as the digital data must be converted to an analog signal before transmission.
...
After the implementation in a system design has been demonstrated now the concept of Riemann Pump itself is explained.\\
...
\end{equation} %%, \hspace{1cm} T = \frac{2*OSR}{f_{sample}
 is calculated basically by integrating the current over time.\\
...
This three bit resolution leads to eight different slopes which are used for the digital to analog conversion process.
...
Corresponding to the demonstrated eight different slopes on the left side, a table is presented which states the corresponding Riemann Code for each slope on the right side.
By the use of this digital code to control the switches, an output voltage can be generated.
Due to a short sampling time, so that the output capacitance is neither fully charged nor discharged, arbitrary signals can be generated.
To reach modern \gls{ab:rf} standards, a minimum amount of resolution bits and of a oversampling ratio has to be fulfilled.
Oversampling is crucial for the recovery of a signal, due to the Nyquist-Shannon sampling theorem (reference???).
% ################################# 16:14 22.04.2016
The Nyquist-Shannon theorem %% Nyquist rate, Nyquist theorem... !!
\begin{equation}
f_{nyquist} \geq 2 f_{signal,max}
\end{equation}
\label{eq:SamplingTheorem}
states, that the sampling rate \gls{sy:fsampling} has to be at least two times the maximum signal frequency.\\
Every increase of \gls{sy:fsampling} above the  \ref is called oversampling ratio \gls{ab:osr}.
The \gls{ab:osr} is defined as:
\begin{equation}
OSR = 2^{r}
\end{equation}
%%%
 The working principle is to integrate a current into a capacitive load, this integration is based on Riemann Integral, where the name come from. This integration converts the current into a voltage. 
...
Implemented in one component this concept saves area and costs and the amplified signal at the drain can be transmitted via the antenna. 
 This output voltage can be applied to the input of a power amp and then to the antenna to propagate it. The current, which charges the capacitive input impedance of the power amp, is controlled by a digital code. A fixed set of slopes, represents the different current sources. A desired signal in the time-domain is generated with MatLab. This signal can consist of many different signals (different carriers and modulation types). This signal is sampled with the given set of slopes. The minimization of the error leads to the Riemann Code. With this Riemann Code (digital) the driver circuit is controlled. This leads to an analog signal formed by the digital input signal. 
