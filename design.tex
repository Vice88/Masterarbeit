...
Considering a \SI{20}{\watt} power amplifier for transmission purposes, led to a \gls{ab:gan} \gls{ab:hemt} with a total gate periphery of \SI{4}{\milli \metre}, based on the approximation \SI[per-mode=fraction]{5}{\watt\per\milli\metre} [\textbf{R. Quay - Gallium Nitride Electronics REFERENCE}].

Due to the non-linearity of the transistor model \cite{vitanovnon}, the input impedance of the power stage is simulated.
The bias voltages of the modelled power transistor were tuned with respect to \gls{ab:mag} to get its input impedance.
